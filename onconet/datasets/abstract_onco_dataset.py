...
class Abstract_Onco_Dataset(data.Dataset):
    ...
    def __init__(self, args, transformers, split_group):
        ...
        self.image_loader.is_training = (split_group == 'train')
    ...
    def get_image_item(self, index):
        ...
        try:
            if self.args.multi_image:
                ...
                # NEW: if the paths are DICOM, load each as (T,C,H,W) and stack views -> (Nviews,T,C,H,W)
                if isinstance(sample['paths'], (list, np.ndarray)) and len(sample['paths']) > 0 and str(sample['paths'][0]).lower().endswith(".npz"):
                    x = self.image_loader.get_dicom_volumes_as_2d_batches(sample['paths'], additionals)
                else:
                ...
                # NEW: single-image DICOM path
                if str(sample['path']).lower().endswith(".npz"):
                    x = self.image_loader.get_dicom_slices_as_2d_batch(sample['path'], additional)
                else:
